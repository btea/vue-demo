<template>
	<div class="container">
		<div class="float-box">
			...
			<ul class="style">
				<li v-for="s in mapStyle" :key="s.v" :class="{ active: s.v === styleV }" @click="mapStyleChange(s.v)">
					{{ s.name }}
				</li>
			</ul>
			<div class="2d-3d">
				3D<el-switch v-model="is3D" active-color="#66ccff" inactive-color="#909399" @change="mapModelSwitch">
				</el-switch>
			</div>
		</div>
	...
	methods: {
		...
		renderMap() {
			var map = new AMap.Map(
				document.getElementsByClassName('map-box')[0],
				this.is3D
					? {
