<template>
	<div>
		<div class="container">
			...
			<div
				v-for="(cell, index) in gridNum"
				:key="index"
				:class="{ goal: position.includes(index), blue: clickList[index].clicked, mark: clickList[index].mark }"
				@click="around(index, 'i')"
				@contextmenu.prevent="mark(index)"
			></div>
		</div>
	</div>
</template>
